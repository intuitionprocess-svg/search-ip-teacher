// 3-digit zip prefix → state abbreviation (covers all US zip ranges)
export const ZIP_PREFIX_STATE: Record<string, string> = {
...
export function stateFromZip(zip: string): string {
  ...
  const abbr = ZIP_PREFIX_STATE[zip.slice(0, 3)] ?? '';
  return STATE_ABBREV[abbr] ?? '';
}

export function stateFromCoords(lat: number, lng: number): string {
  let best = '';
  let bestDist = Infinity;
  for (const [state, [clat, clng]] of Object.entries(STATE_CENTROIDS)) {
